...
int gen(struct constraint input_parameter) {
	...
	if ((func = fopen("log10_gen.c", "w")) == (FILE *)0) {
	...
	*/
	...
	// generate code for exp_gen
	{
		...
		fprintf(func, "\n");
		fprintf(func, "static const DL	log10_t =\n");
		fprintf(func, "{ .l = 0x3fdbcb7b1526e50e };\n");
		fprintf(func, "\n");
		// degree
		if (degree > 0)	{
	...
	{
		fprintf(func, "double log10_gen(double x) {\n");
		fprintf(func, "	_UL ix;\n");
		...
		if (degree > 1) {
			...
			fprintf(func, "	result = result * log10_t.d;\n");
		}
		else if (degree == 1) {
			fprintf(func, "	l_lead = logtabhi[k].d;\n");
			fprintf(func, "	l_trail = logtablo[k].d;\n");
