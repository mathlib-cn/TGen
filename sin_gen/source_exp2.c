...
int gen(struct constraint input_parameter) {
	...
	if ((func = fopen("exp2_gen.c", "w")) == (FILE *)0) {
	...
	*/
	...
	// generate code for exp_gen
	{
		...
		fprintf(func, "\t{.l = 0x%lx}\n", interpolate[i * (1 << (BIT - bit))].l);
		fprintf(func, "};\n\n");
		fprintf(func, "static const DL ln2 = \n");
		fprintf(func, "{ .l = 0x3fe62e42fefa39ef };\n");
		fprintf(func, "\n");
	}
	{
		fprintf(func, "double exp2_gen(double x) {\n");
		fprintf(func, "	double T, r, r_poly, result, r_hi, r_lo;\n");
		fprintf(func, "	long int T_int;\n");
		fprintf(func, "	unsigned long int hi, lo;\n");
		fprintf(func, "\n");
		fprintf(func, "	T = x * BITNUM;\n");
		fprintf(func, "	T_int = T;\n");
	
		if (degree > 1) {
			fprintf(func, "	// 0 <= r <= 1/k1\n");
			fprintf(func, "	r = x - T_int / (double)(BITNUM);\n");
			fprintf(func, "	r = r * ln2.d;\n");
			fprintf(func, "\n");
