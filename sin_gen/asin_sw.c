...
// P = fpminimax(asin(x), [|1,3,5,7,9,11,13,15|], [|D...|], [-sqrt(1-(63/64)*(63/64)),sqrt(1-(63/64)*(63/64))]); 
// x * (0x3ff0000000000000 + x ^ 2 * (0x3fc5555555555579 + x ^ 2 * (0x3fb33333333251c1 + x ^ 2 * (0x3fa6db6db8c030c4 + x ^ 2 * (0x3f9f1c6fd80c3571 + x ^ 2 * (0x3f96e94150979658 + x ^ 2 * (0x3f91b0b76412beaf + x ^ 2 * 0x3f8fb38ec5ea5bc4)))))))
/*
static const DL
P[] = {
	...
	{.l = 0x3f96e94150979658},
	{.l = 0x3f91b0b76412beaf},
	{.l = 0x3f8fb38ec5ea5bc4},
};
*/
...
double asin_gen(double x)
{
	...
	// this way's performance maybe better, and max&average precision are the same to the following way. But maybe not easy to auto generate
	// result = delta * wsq * (C[2].d + wsq * (C[4].d + wsq * C[6].d)) + delta * zsq * (C[1].d + wsq * (C[3].d + wsq * (C[5].d + wsq * C[7].d)));
	//double wsq = zsq * zsq;
	//result = wsq * (C[2].d + wsq * (C[4].d + wsq * C[6].d)) + zsq * (C[1].d + wsq * (C[3].d + wsq * (C[5].d + wsq * C[7].d)));
	//result = asin_tab[order].d + (delta + result * delta);

	poly = ((((((P[7].d*zsq + P[6].d)*zsq + P[5].d)*zsq + P[4].d)*zsq + P[3].d)*zsq + P[2].d)*zsq +	P[1].d)*(zsq*z) + z;
