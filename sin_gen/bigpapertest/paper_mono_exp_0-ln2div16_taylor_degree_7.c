...
C[DEGREE] = {
	...
	{.l = 0x3f81111111111111 },
	{.l = 0x3f56c16c16c16c17 },
	{.l = 0x3f2a01a01a01a01a },
};
...
double exp_gen(double x) {
	...
	double result;
	double r_hi, r_lo;
	unsigned long int step = 1 << (7 - BIT);
	...
	//rr = r * r;
	r_poly = (r * r) * (C[2].d + r * (C[3].d + r * (C[4].d + r * (C[5].d + r * (C[6].d + r * (C[7].d)))))) - r2 * C[1].d + r1 * C[1].d;

	lo = (T_int & BITNUM_1) * step;
	hi = T_int >> BIT;
	hi = (hi + 0x3ff) << 52;
	r_hi = *((double *)&hi);
