...
int gen(struct constraint input_parameter) {
	...
	if ((func = fopen("log2_gen.c", "w")) == (FILE *)0) {
	...
	*/
	...
	// generate code for exp_gen
	{
		...
		fprintf(func, "\n");
		fprintf(func, "static const DL	log2_t =\n");
		fprintf(func, "{ .l = 0x3ff71547652b82fe };\n");
		fprintf(func, "\n");
		// degree
		if (degree > 0)	{
	...
	{
		fprintf(func, "double log2_gen(double x) {\n");
		fprintf(func, "	_UL ix;\n");
		...
		if (degree > 1) {
			...
			fprintf(func, "	result = m + result * log2_t.d;\n");
		}
		else if (degree == 1) {
			fprintf(func, "	l_lead = logtabhi[k].d;\n");
			fprintf(func, "	l_trail = logtablo[k].d;\n");
